...
### 之前的单页应用React对于大型项目的局限性思考(自我构思)

可以将我们之前所做的index.html看成是一个大型应用中的一个导航链接,我们有很多这样的index.html页面可以渲染, 在服务端渲染的时候走不同的路由进行渲染(根据url进行不同页面的渲染). index.html页面内仍然使用react-router.但是该单页应用作为一个导航可以被a标签链接. 当然如果不用服务端渲染,那么就要使用客户端LocalStorage或者SessionStorage来保持state状态,防止刷新的时候状态被初始化.


### 结论

React学习路线比较陡峭,不容易上手,而且对于大型的项目需求设计可能也存在一些弊端(但是这个问题好像并不是问题,而且在单页应用的首次渲染性能上可能React更胜Vue).React能火最大一部分是因为移动端的React-Native.Vue的渲染性能在React之上,而且之后还有适应于三种平台的weex扩展(weex已经在淘宝,天猫等大型移动项目上经过考验,毕竟他们之前想用react-native来做结果发现存在很多问题),目前而言是值得学习的一个框架,并且比React容易上手.
...
学习Go语言是有需求的,但不是必要的,可以私下里学习.目前Node在业务上足可以满足需求(毕竟组内也没有人接触过Go语言,对于Node更为熟悉).而且Go语言学习可能需要一个长期积累的过程,毕竟是一门新的语言,不过在Go学习上还是有优势的,毕竟更接近C语言.
